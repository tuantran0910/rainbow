{% macro lambda_fact_sales(batch_orders_relation, batch_order_items_relation, stream_orders_relation, stream_order_items_relation, cutoff_days=1) %}

    {# Calculate cutoff timestamp for lambda view #}
    {% set cutoff_timestamp = calculate_cutoff_timestamp(cutoff_days=cutoff_days) %}

    WITH
        -- Batch layer orders and order_items (T-1)
        batch_orders AS (
            SELECT *
            FROM {{ batch_orders_relation }}
            WHERE created_date_tz_hcm < DATE({{ cutoff_timestamp }})
        ),

        batch_order_items AS (
            SELECT *
            FROM {{ batch_order_items_relation }}
            WHERE created_date_tz_hcm < DATE({{ cutoff_timestamp }})
        ),

        -- Stream layer orders and order_items (T)
        stream_orders AS (
            SELECT *
        ...
        -- Unified orders: combine batch and stream
        unified_orders AS (
            SELECT *
            FROM stream_orders
            ...
            LEFT JOIN stream_orders AS stream
                ON batch.id = stream.id
            WHERE stream.id IS NULL
        ),

        -- Unified order_items: combine batch and stream
        unified_order_items AS (
            SELECT *
            FROM stream_order_items
            
            UNION ALL
            
            SELECT batch.*
            FROM batch_order_items AS batch
            LEFT JOIN stream_order_items AS stream
                ON batch.id = stream.id
            WHERE stream.id IS NULL
        ),
    ...
    FROM unified_orders AS orders
    LEFT JOIN unified_order_items AS order_items
