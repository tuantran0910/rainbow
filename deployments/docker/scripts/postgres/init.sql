...
-- Grant necessary connection privileges (can be done before connecting to dagster)
...
-- Initialize the necessary resources for the API (connected to dagster now)
...
-- Grant necessary connection privileges (can be done before connecting to rainbow)
...
-- Grant necessary connection privileges (can be done before connecting, but we are already connected to rainbow)
...
-- Grant schema and table privileges (MUST be done while connected to 'rainbow')
...
-- Set the default privileges for future tables (MUST be done while connected to 'rainbow')
ALTER DEFAULT PRIVILEGES IN SCHEMA public
GRANT SELECT ON TABLES TO cdc;

-- Create the publication for logical replication (MUST be done while connected to 'rainbow')
...
-- Initialize the necessary resources for Metabase (connected to rainbow now)
...
-- Grant necessary connection privileges (can be done before connecting to metabase)
...
-- Grant necessary connection privilege to dagster database (can be done before connecting to dagster)
GRANT CONNECT ON DATABASE dagster TO el;

-- *** CRITICAL FIX: Switch to the 'dagster' database BEFORE granting privileges *within* it ***
\echo 'Connecting to dagster database for EL permissions...'
\c dagster

-- Grant schema and table privileges (MUST be done while connected to 'dagster')
\echo 'Granting EL permissions within dagster database...'
GRANT USAGE ON SCHEMA public TO el;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO el;

-- Ensure future tables are also accessible (MUST be done while connected to 'dagster')
ALTER DEFAULT PRIVILEGES FOR ROLE dagster IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO el;
