...
resource "google_compute_global_address" "superset_ingress_ip" {
  name         = "superset-ingress-ip"
  address_type = "EXTERNAL"
}
...
# DNS record for Superset ingress
resource "google_dns_record_set" "superset_dns" {
  name         = "superset.tuantrann.work."
  managed_zone = data.google_dns_managed_zone.tuantrann_work.name
  type         = "A"
  ttl          = 300
